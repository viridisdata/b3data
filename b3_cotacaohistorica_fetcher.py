...
def valid_date(datetuple: DateTuple) -> bool:
    ...
    workday = weekday != 5 and weekday != 6
    brazillian_holidays = (
        ...
        dt.date(year, 9, 7),    # 7 de setembro (Dia da Independência do Brasil)
    ...
    not_holiday = not any(date == holiday for holiday in brazillian_holidays)
    return workday and not_holiday
...
def get_filename(datetuple: DateTuple, modified: dt.datetime = None) -> str:
    year, month, day = datetuple
    if month is None:
        return f"b3-cotacaohistorica_{year}_{modified:%Y%m%d}.zip"
    elif day is None:
        return f"b3-cotacaohistorica_{year}{month:02}_{modified:%Y%m%d}.zip"
    else:
        return f"b3-cotacaohistorica_{year}{month:02}{day:02}_{modified:%Y%m%d}.zip"
...
def fetch_data_file(datadir: Path, datetuple: DateTuple, client: httpx.Client = None, blocksize: int = 8192) -> None:
    ...
    if r.status_code != 200:
        ...
        raise Exception(f"Error fetching {url}: {r.headers.get('content-type')}")
